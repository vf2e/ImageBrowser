...
- **AI 美学评分**：集成 [EAT 模型](https://github.com/woshidandan/Image-Aesthetics-and-Quality-Assessment)，照片左上角显示 1–10 分美学评分（一键安装，见下方）。
...
1. 自动克隆 EAT 代码、创建 Python 环境  
2. 将下载的权重放入 `aesthetics\weights\finetune.pth`  
3. 正常启动 `ImageBrowser.exe` 即可，**无需配置环境变量**

详见 [docs/aesthetics.md](docs/aesthetics.md)
...
```
ImageBrowser/
├── aesthetics/                 # EAT 美学模型（setup 后生成 eat-repo、venv）
│   ├── eat_server.py
│   ├── requirements.txt
│   └── weights/                # 放入 finetune.pth
├── src/                        # C++ 源码
...
│   ├── setup_aesthetics.bat    # 一键安装 EAT 美学模型
...
等价于 `scripts\package.bat`。脚本自动完成以下步骤：

1. **Release 构建** — 编译生成 `ImageBrowser.exe`
2. **依赖部署** — 调用 `windeployqt --qmldir qml` 收集 Qt 运行时和 QML 模块
3. **复制美学模块** — 打包 `aesthetics/`（含权重、venv、EAT 代码，若已配置）
4. **生成便携 ZIP** — 输出到 `output\ImageBrowser_v1.0.0_portable.zip`
5. **生成安装包** — 调用 Inno Setup 编译 `installer\ImageBrowser.iss`（若已安装）
...
| `output\ImageBrowser_v1.0.0_portable.zip` | 便携压缩包 |
...
若未检测到 Inno Setup，脚本仍会完成构建、`dist` 部署和 ZIP，并提示手动编译 `installer\ImageBrowser.iss`。
